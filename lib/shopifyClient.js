...
async function generateCodeChallenge(codeVerifier){
    const digestString = await Crypto.digestStringAsync(Crypto.CryptoDigestAlgorithm.SHA256, codeVerifier);

    return Buffer.from(digestString, 'hex').toString('base64') // Use 'base64' instead of 'base64url'
    .replace(/\+/g, '-') // Convert to base64url
    .replace(/\//g, '_')
    .replace(/=/g, '');
}

export async function buildAuthorizationUrl(codeVerifier){
