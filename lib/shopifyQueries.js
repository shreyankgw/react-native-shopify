...
export async function recentlyViewedProducts(products) {
  const query = `
  query recentlyViewed($query: String!){
  ...
  const { data, errors } = await client.request(query, { variables: { query: products } });

  if (errors) {
    throw new Error(errors[0])
  }

  const recentlyViewed = data.products;
