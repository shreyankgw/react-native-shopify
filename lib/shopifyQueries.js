...
export async function fetchCollection(handle, productsNum = 24, cursor = null, sortBy = "BEST_SELLING", selectedFilters = {}) {
  const filtersArray = Object.entries(selectedFilters).flatMap(([key, values]) =>
    values.map(value => ({ key, value }))
  );

    const query = `
      query collection($handle: String, $productsNum: Int!, $cursor: String, $sortKey: ProductCollectionSortKeys, $filters: [ProductFilter!]){
        collection(handle: $handle){
          title
          description
          products(first: $productsNum, after: $cursor, sortKey: $sortKey, filters: $filters){
            filters{
               id
               label
               type
